...
if (file_exists("../config.php") && $mOverwrite != 1) {
    $mWsRetObj->setFailure(WsErrors::fileAlreadyExists, getFn(__FILE__));
} else if (testDb($mDbHost, $mDbUsr, $mDbPwd)) {
    $mWsRetObj->addMsg(WsErrors::overwritingFile, getFn(__FILE__));
    if ($mAppTitle && $mDb && $mDbHost && $mDbUsr && $mDbPwd) {
        ...
        file_put_contents("../config2.php", $mConfigPHP, LOCK_EX);
    } else {
        $mWsRetObj->setFailure(WsErrors::reqParamMissing, getR($_POST));
    }
} else {
    $mWsRetObj->setFailure(WsErrors::couldNotConnectToDatabase);
}
